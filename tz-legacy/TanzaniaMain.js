...
var RosterClientVal = function (AccNum){
    if (typeof AccNum === 'undefined' || AccNum == ''){
        ...
        if (AccNum.length == 8){
            ...
            var response = rosterAPI.authClient(AccNum,'TZ');
            return response;
        }
        else {
            return false;
...
var RosterClientGet = function (AccNum){
    rosterAPI.verbose = true;
    rosterAPI.dataTableAttach();
    var client = rosterAPI.getClient(AccNum,'TZ');
    return client;
};
...
var GetHeathyPathPercent = function (Season,RegionName){
    ...
    var cursorRegion = table.queryRows({
    ...
    if (cursorRegion.hasNext()){
        ...
        var cursorDefault = table.queryRows({
...
addInputHandler('SplashMenu', function(SplashMenu) {
    ...
    } else if(SplashMenu == 1) {
        sayText('Weka namba ya akaunti');
        promptDigits('account_number', {submitOnHash: true, maxDigits: 8, timeout: 5});
    }
});
...
addInputHandler('account_number', function(accountNumber) {
    ...
    if (RosterClientVal(ClientAccNum)){
        ...
        var client = RosterClientGet(ClientAccNum);
