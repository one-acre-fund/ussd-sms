module.exports = {
    'prev_page': {
        'en': '44)Previous page',
        'ki': '44)Gusubira inyuma'
    },
    'next_page': {
        'en': '77)Next page',
        'ki': '77)Gukomeza'
    },
    'ext_farmer_national_id': {
        'en': 'Enter the farmer National ID Number',
        'ki': 'Andika nimero y\' irangamuntu y\' umuhinzi',
    },
    'ext_farmer_national_id_used': {
        'en': 'The National ID Number is already registered',
        'ki': 'Nimero y\' irangamuntu y\' umuhinzi mwinjije iranditse',
    },
    'ext_farmer_name_1': {
        'en': 'Enter the farmer\'s first name',
        'ki': 'Andika izina ry\'ikinyarwanda ry\' umuhinzi',
    },
    'ext_farmer_name_2': {
        'en': 'Enter the farmer\'s second name',
        'ki': 'Andika izina ry\'irikirisitu ry\' umuhinzi',
    },
    'exiting': {
        'en': 'Thank you for using the service. Exiting',
        'ki': 'Murakoze gukoresha iyi serivice',
    },
    'ext_farmer_gender': {
        'en': 'What gender is the Farmer who will receive the tester pack?\n1) Male\n2) Female',
        'ki': 'Igitsina cy\'umuhinzi uzakira inyongeramusaruro z\'igerageza\n1) Gabo\n2) Gore'
    },
    'ext_farmer_phone': {
        'en': ' What the phone number that the farmer can be reached on?',
        'ki': ' Nimero ya telefone uyu muhinzi yabonekaho?'
    },
    'ext_farmer_not_eligible': {
        'en': 'Thank you, this farmer is not eligible',
        'ki': 'Murakoze, uyu muhinzi ntiyujuje ibisabwa'
    },
    'ext_farmer_confirmation': {
        'en': 'Thank you for registering and confirming that the farmer has received the pack',
        'ki': 'Murakoze, Uyumuhinzi yanditswe kandi wemeje ko yakiriye inyongera musaruro zigerageza'
    },
    'ext_farmerId_used_NE': {
        'en': 'The farmer\'s Id was used and they are not eligible',
        'ki': 'Nimero y\' irangamuntu y\' umuhinzi yakoreshejwe, ntago yujuje ibisabwa'
    },
    'ext_farmerId_used_RE': {
        'en': 'The farmer\'s Id was used and they are already registered',
        'ki': 'Nimero y\' irangamuntu y\' umuhinzi yakoreshejwe, ari kurutonde rwabazahabwa inyongera musaruro zigerageza'
    },
    'ext_invalid_national_id': {
        'en': 'The national Id entered is incorect. Please try again',
        'ki': ' Inomero y\'irangamuntu mwayanditse nabi. Ongera ongerageze'
    },
    'ext_main_splash': {
        'en': 'Welcome. ~B',
        'ki': 'Murakaza neza. ~B'
    },
    'fp_enter_id': {
        'en': 'Welcome Farmer Promoter. Enter your village ID',
        'ki': 'Ikaze mujyanama, andika code y\'umudugudu wawe'
    },
    'sedo_enter_id': {
        'en': 'Please identify yourself with your SEDO ID (6 digits).',
        'ki': 'Subiza na code yawe ya SEDO (Imibare 6)?'
    },
    'sedo_enter_vid': {
        'en': 'Please reply with your village ID',
        'ki': 'Subiza na code y\'umudugudu wasuye'
    },
    'invalid_fp_age': {
        'en': 'Not correct, please reply with a number ~BWhat is the Farmer Promoter\'s Age?',
        'ki': 'Sibyo, subiza wandika umubare~BImyaka y\'umujyanama w\'ubuhinzi?'
    },
    'sedo_invalid_id': {
        'en': 'This is an invalid SEDO ID. Please try again',
        'ki': 'Ntabwo wanditse neza code yawe. Ongera usubiremo'
    },
    'invalid_vid': {
        'en': 'This is not a valid village ID, please try again.',
        'ki': 'Iyo ntabwo ari code y\'umudugudu. ongera usubiremo'
    },
    'confirm_crop': {
        'en': 'You selected $CROPNAME.~B1) Confirm~B0)Don\'t confirm',
        'ki': 'Hitamo $CROPNAME.~B1) Niba aribyo~B0) Niba ataribyo'
    },
    'fp_gender': {
        'en': 'What is the Farmer Promoter\'s gender? ~B1) Male~B2) Female',
        'ki': 'Igitsina y\'umujyanama w\'ubuhinzi ~B1) Gabo ~B2) Gore'
    },
    'fp_age': {
        'en': 'What is the Farmer Promoter\'s Age?',
        'ki': 'Imyaka y\'umujyanama w\'ubuhinzi?'
    },
    'fp_tenure': {
        'en': 'How many years has the Farmer Promoter been a Farmer Promoter',
        'ki': 'Umujyanama amaze igihe kingana iki ari umujyanama w\'ubuhinzi?'
    },
    'fp_num_trained': {
        'en': 'How many farmers did the FP practically train this season in the field?',
        'ki': 'Umujyanama w\'ubuhinzi yahuguye abahinzi bangahe mumurima?'
    },
    'num_groups': {
        'en': 'How many Twigire Groups does the FP have?',
        'ki': 'Umujyanama w\'ubuhinzi afite amatsinda angahe ya Twigire Muhinzi?'
    },
    'fp_num_trainings': {
        'en': 'How many training sessions has the Farmer Promoter received this season?',
        'ki': 'Umujyanama w\'ubuhinzi yahawe amahugurwa inshuro zingahe muri iki gihembwe?'
    },
    'survey_start': {
        'en': 'Choose a crop:~B',
        'ki': 'Hitamo igihingwa:~B'
    },
    'survey_question': {
        'en': '$FEEDBACK~B$TEXT~B$OPT1~B$OPT2~B$OPT3~B$OPT4~B5) Ntabyo nzi',
        'ki': '$FEEDBACK~B$TEXT~B$OPT1~B$OPT2~B$OPT3~B$OPT4~B5) Ntabyo nzi'
    },
    'invalid_input': {
        'en': 'Please try again',
        'ki': 'Ongera usubiremo'
    },
    'closing_message': {
        'en': 'Thank you for answering the questions. You answered $NUM_CORRECT out of 10 correctly.',
        'ki': 'Murakoze gusubiza ibibazo. Mubibazo mwakoze mubashyije gusubiza $NUM_CORRECT kuri 10 neza. Twigire Muhizi izifashisha ibisubizo mukunoza imikorere.'
    },
    'fp_trained': {
        'en': 'Did the FP receive training this season on how to plant this crop?~B1) Yes~B2) No',
        'ki': 'Umujyanama w\'ubuhinzi yabonye amahugurwa iki gihembwe kuri iki gihingwa?~B1) Yego~B2) Oya'
    },
    'num_females_crop': {
        'en': 'How many unique female farmers were practically trained this season on this crop?',
        'ki': 'Umujyanama w\'ubuhinzi yahuguye abagore bangahe iki gihembwe kuri iki gihingwa?'
    },
    'num_males_crop': {
        'en': 'How many unique male farmers were practically trained this season on this crop?',
        'ki': 'Umujyanama w\'ubuhinzi yahuguye abagabo bangahe iki gihembwe kuri iki gihingwa?'
    },
    'num_farmers': {
        'en': 'How many unique farmers were trained at a demo plots on planting?',
        'ki': 'Umujyanama w\'ubuhinzi yahuguye abahinzi bangahe bose mu gihe cy\'itera ku murima ntangarugero ?'
    },
    'num_plots': {
        'en': 'How many demo plots does the Farmer Promoter have?',
        'ki': 'Umujyanama w\'ubuhinzi afite imirima ntangarugero ingahe?'
    },
    'num_females': {
        'en': 'How many females in Twigire Groups in this village?',
        'ki': 'Mu matsinda ya twigire harimo abagore bangahe mu mudugudu ?'
    },
    'num_males': {
        'en': 'How many males in Twigire Groups in this village?',
        'ki': 'Mu matsinda ya twigire harimo abagabo bangahe  mu mudugudu?'
    },
    'fp_gender_tra': {
        'en': 'What is your gender? ~B1) Male~B2) Female',
        'ki': 'Igitsina ~B1) Gabo~B2) Gore'
    },
    'fp_age_tra': {
        'en': 'What is your age?',
        'ki': 'Ufite imyaka ingahe?'
    },
    'fp_tenure_tra': {
        'en': 'How many years have you been a Farmer Promoter?',
        'ki': 'Umaze igihe kingana iki uri umujyanama ?'
    },
    'fp_tr_num': {
        'en': 'What is your trainer number?',
        'ki': 'Andika nimero y\'ugiye ku guhugura'
    },
    'crop_type_menu': {
        'en': 'Which crop?~B $CropType',
        'ki': 'Ikihe gihingwa:~B $CropType'
    },
    'invalid_entry': {
        'en': 'You have entered an incorect input enter 1 to retry or 99 exit',
        'ki': ' Mwinjije ibitaribyo, kanda 1 kugirango wongere ugeraze 99 kuvamo',
    },
    'crop_amount_menu': {
        'en': 'Quantity of this crop to sell in kg',
        'ki': 'Ingano y\'umusaruro ugurisha mu biro'
    },
    'price_floor_menu': {
        'en': 'Lowest price per kg',
        'ki': 'Igiciro cyo hasi ku kiro'
    },
    'harvest_timing_menu': {
        'en': 'When Maize will be available~B $HarvestTime',
        'ki': 'Igihe umusaruro uzabonekera~B $HarvestTime'
    },
    'market_access_confirmation_menu': {
        'en': 'You are committing to sell $Amount kgs of unshielded maize in $Timeline timeframe. Enter 0 to confirm',
        'ki': 'Mwemeye kugurisha imifuka $Amount, y\'ibigoli bidahunguye (biri kubitiritiri) mu gihe cy\'$Timeline. Kanda 0 Wemeze'
    },
    'market_access_final_message': {
        'en': 'Thank you for using this service. If you want to go to the main menu press 0. Tubura your trusted patner',
        'ki': 'Murakoze gukoresha iyi serivice. Niba ushaka gusubira ahabanza kanda 0. Tubura umufatanyabikorwa wanyu wizewe'
    },
    'invalid_try_again': {
        'en': 'Invaid input, please try again. ~B $Menu',
        'ki': 'Ibyo mwinjije ntago bishoboka, ongera ugerageze. ~B $Menu'
    },
    'cor_main_splash': {
        'en': 'Welcome to TUBURA. Please enter your account number',
        'ki': 'Murakaza neza muri TUBURA. Andika nimero ya konti y\'umuhinzi:',
    },
    'account_number_verified': {
        'en': 'account number received',
        'ki': 'Nomero ya konti yakiriwe',
    },
    'cor_get_balance': {
        'en': 'Hello $CLIENT_NAME~BCredit:$CREDIT~BPaid:$PAID~BBalance:$BALANCE~B$HEALTHY_PATHAccurate as of $DAY_NAME $DAYNR $MONTH',
        'ki': 'Muraho $CLIENT_NAME~BIdeni ryose:$CREDIT~BAyishyuwe:$PAID~BIdeni risigaye:$BALANCE~B$HEALTHY_PATHAmakuru yafashwe $DAY_NAME $DAYNR $MONTH',
    },
    'cor_get_balance_overpaid': {
        'en': 'Hello $CLIENT_NAME~BCredit:$CREDIT~BPaid:$PAID~BBalance:$BALANCE~BOverpaid amount: $OVERPAID~B$HEALTHY_PATHAccurate as of $DAY_NAME $DAYNR $MONTH',
        'ki': 'Muraho $CLIENT_NAME~BIdeni ryose:$CREDIT~BAyishyuwe:$PAID~BIdeni risigaye:$BALANCE~BAmafaranga arenga: $OVERPAID~B$HEALTHY_PATHAmakuru yafashwe $DAY_NAME $DAYNR $MONTH',
    },
    'external_splash': {
        'en': 'Welcome to TUBURA. Please select your region:~B$GEO_MENU',
        'ki': 'Murakaza neza muri TUBURA! Nyabuneka hitamo intara yawe:~B$GEO_MENU',
    },
    'exit': {
        'en': 'Goodbye',
        'ki': 'Murakoze',
    },
    'geo_selections': {
        'en': '$GEO_MENU',
        'ki': '$GEO_MENU',
    },
    'group_size_not_satisfied': {
        'en': 'The group has reached its maximum of 16 people, find a different group to register with',
        'ki': ' Itsinda mwinjije rwujuje umubare wabantu 16 usabwa, shaka irindi tsinda wiyandikishamo'
    },
    'cto_fo_information': {
        'en': 'Your FO is $FO_NAME~BTheir phone number is $FO_PHONE',
        'ki': 'Umukangurambaga wawe ni $FO_NAME.~BNomero ya telefone ye ni $FO_PHONE',
    },
    'cto_no_fo': {
        'en': 'TUBURA is not currently in $CELL_NAME. We\'ll keep your info on file',
        'ki': 'Mwihangane ntabwo dukorera mu gace kanyu. Turizera neza ko tuzagurira ibikorwa byacu muri ako gace vuba aha kandi tuzakomeza kuguha amakuru',
    },
    'invalid_geo_input': {
        'en': 'Invalid selection - please try again. ~B1) Continue ~B99) Exit',
        'ki': 'Ibyo wahisemo ntibibashije kuboneka- ongera ugererageze~B1) Gukomeza~B99) Kuvamo',
    },
    'cto_fo_message_content': {
        'en': 'Call $CLIENT_PHONE - potential new client',
        'ki': 'Ufite abahinzi bashya beza-hamagara $CLIENT_PHONE',
    },
    'enr_splash': {
        'en': '$ENR_SPLASH',
        'ki': '$ENR_SPLASH',
    },
    'enr_quantity_not_in_the_list': {
        'en': 'The quantity entered is not available. These are the allowed quantities: $QUANTITY_LIST',
        'ki': 'Ingano mwinjije ntago ishoboka. Izi nizo zishoboka $QUANTITY_LIST',
    },
    'enr_reg_start': {
        'en': 'Please enter NID',
        'ki': 'Andika nomero y\'irangamuntu',
    },
    'enr_nid_confirm': {
        'en': 'Confirm NID',
        'ki': 'Ongera wandike wemeza inomero y\'irangamuntu',
    },
    'enr_name_1': {
        'en': 'Enter first name',
        'ki': 'Andika izina ryawe ry\'ikinyarwanda',
    },
    'enr_name_2': {
        'en': 'Enter second name',
        'ki': 'Andika izina ryawe ry\'irikirisitu',
    },
    'enr_pn': {
        'en': 'Enter phone number',
        'ki': 'Andika nomero ya telefone',
    },
    'enr_glus': {
        'en': 'Enter GL or US id',
        'ki': 'Andika nomero y\' umukuru w\'itsinda mu iyandikwa',
    },
    'enr_tc': {
        'en': 'Confirm that client has read TUBURA terms and conditions (THIS IS BINDING)~B1) Confirm~B99)Exit',
        'ki': 'Emeza nimba umuhinzi yamaze gusoma amategeko n\'amabwiriza ya TUBURA (Imbere n\'inyuma)~B1) Gukomeza~B99) Kuvamo',
    },
    'enr_gc': {
        'en': 'Confirm that client is on group constitution',
        'ki': 'Emeza nimba umuhinzi ari ku itegeko rigenga itsinda',
    },
    'enr_sns': {
        'en': 'Confirm that client has registered in SNS',
        'ki': 'Emeza nimba yamaze kwiyandikisha muri SNS',
    },
    'enr_order_start': {
        'en': 'Please enter account number',
        'ki': 'Andika nomero ya Konti y\'umuhinzi',
    },
    'enr_order_review_start': {
        'en': 'Please enter account number',
        'ki': 'Andika nomero ya Konti y\'umuhinzi',
    },
    'enr_glus_id_start': {
        'en': 'please enter your national id number to retrieve your gl id',
        'ki': 'Andika nomero y\'irangamuntu yawe ubone nomero iranga umukuru w\'itsinda ryawe',
    },
    'enr_finalize_start': {
        'en': 'Please enter your account number to finalize your order',
        'ki': 'Andika nomero ya Konti yawe wemeze neza ibyo watumije',
    },
    'invalid_input_ext': {
        'en': 'Invalid selection - please try again~B1)Continue~B99)Exit',
        'ki': 'Ibyo wahisemo ntibibashije kuboneka~B1) Gukomeza~B99) Kuvamo',
    },
    'account_number_not_found': {
        'en': 'account number not found pleas try again',
        'ki': 'Nomero ya konti ntabwo ibonetse ongera ugerageze',
    },
    'enr_nid': {
        'en': 'Please enter NID',
        'ki': 'Andika nomero ndangamuntu yawe',
    },
    'enr_invalid_nid': {
        'en': 'invalid nid please reenter',
        'ki': 'Nomero ndangamuntu uyanditse nabi,ongera',
    },
    'enr_unmatched_nid': {
        'en': 'nids do not match please reenter nids',
        'ki': 'Nomero ndangamuntu wanditse ntihura n\'iyambere,yandike neza',
    },
    'invalid_pn_format': {
        'en': 'invalid pn format. please reenter pn',
        'ki': 'Nomero ya Telephone yanditse nabi.Ongera wandike neza',
    },
    'enr_invalid_glus': {
        'en': 'invalid glus id try again',
        'ki': 'Nomero iranga umuyobozi w\'itsinda wanditse siyo. Ongera ugerageze',
    },
    'enr_reg_complete': {
        'en': 'Thank you for enrolling with TUBURA! Your account number is $ACCOUNT_NUMBER. Please save this! 1) Continue Ordering',
        'ki': 'Murakoze kwiyandikisha, $NAME. Bwira umuhinzi nimero ya konti ya TUBURA: $ACCOUNT_NUMBER. Subiza 1) Gutumiza',
    },
    'enr_bad_input_increment': {
        'en': 'invalid increment~B1)Continue~B99)Exit',
        'ki': 'Iyo ngano ntabwo ishoboka~B1)Komeza~B99)Gusohoka',
    },
    'enr_confirm_input_order': {
        'en': 'You are ordering $QUANTITY $UNIT of $NAME. This will ad $TOTAL_PRICE to your credit.~B1) Confirm~B44)Go Back',
        'ki': 'Utumije $QUANTITY $UNIT ya $NAME.Ibi bizongera $TOTAL_PRICE ku ideni ryawe~B1)Komeza~B99)Gusohoka~B44)Gusubira inyuma',
    },
    'enr_glus_retrieved': {
        'en': 'your glus is $GLUS',
        'ki': 'Nomero y\'iyandikwa y\'umuyobozi w\'itsinda wawe ni $GLUS',
    },
    'enr_invalid_name_input': {
        'en': 'name is invalid try again',
        'ki': 'Wanditse Izina nabi,Ongera',
    },
    'enr_reg_complete_sms': {
        'en': 'Thank you for enrolling with TUBURA! Your account number is $ACCOUNT_NUMBER. Please save this! $AD_MESSAGE Call 2580 with any questions',
        'ki': 'Murakoze kwiyandikisha muri TUBURA, $NAME! Konte yawe ni $ACCOUNT_NUMBER. Yandike uyizirikane! Nimba ufite ikibazo hamagara 2580',
    },
    'enr_product_selected': {
        'en': 'You have selected $NAME. The cost is $PRICE per $UNIT. Please enter the number of units to order',
        'ki': 'Utumije $NAME. Igiciro ni $PRICE ku $UNIT. Ngaho hitamo ingano wifuza gutumiza',
    },
    'enr_input_out_of_bounds': {
        'en': 'amount selected is out of bounds please try again. maximum is $max',
        'ki': 'Umubare uhisemo ntabwo ushoboka,Ongera uhitemo neza. umubare ntarengwa ni $max',
    },
    'enr_input_order_success': {
        'en': 'Order successfully placed~B1)Continue ordering~B2)confirm order~B3)Main menu~B99)Exit',
        'ki': 'Itumiza ryawe rigenze neza~B1)Komeza gutumiza~B2)Suzuma unemeze ibyo watumije~B3)Ahabanza~B99)Gusohoka',
    },
    'enr_empy_order_review': {
        'en': 'no order yet placed~B1)Continue~B99)Exit',
        'ki': 'Ntacyo uratumiza~B1)Komeza~B99)Gusohoka',
    },
    'enr_invalid_product_selection': {
        'en': 'invalid product selection~B1)Continue~B99)Exit',
        'ki': 'Ibyo uhisemo sibyo~B1)Komeza~B99)Gusohoka',
    },
    'enr_finalize_verify': {
        'en': '1) Confirm finalization~B99)Exit',
        'ki': '1) Emeza ko usoje~B99)Gusohoka',
    },
    'enr_finalized': {
        'en': 'Order finalized. Call 2580 if you wish to change this',
        'ki': 'Ibyo watumije biremejwe.Hamagara 2580 nimba ushaka guhindura',
    },
    'enr_already_finalized': {
        'en': 'Order already finalized. Call 2580 to change~B1)Continue~B99)Exit',
        'ki': 'Ibyo watumije byemejwe mbere.Hamagara 2580 nimba ushaka guhindura~B1)Komeza~B99)Gusohoka',
    },
    'dpm_splash': {
        'en': '$DPM_SPLASH',
        'ki': '$DPM_SPLASH',
    },
    'dpm_reg_start': {
        'en': 'Please enter national ID number',
        'ki': 'Andika numero y\'irangamuntu',
    },
    'enr_no_inputs_for_finalize': {
        'en': 'no inputs selected cannot confirm',
        'ki': 'Ntacyo uzafata wahisemo! Hitamo',
    },
    'dpm_reg_complete': {
        'en': 'Thank you for Enrolling. Give a client this account number $ACCOUNT_NUMBER.~Write OAFID: $OAFID in your trial tracker. ~B1)Continue~B99)Exit',
        'ki': 'Murakoze kwiyandikisha. Bwira umuhinzi nimero ya konti ya TUBURA: $ACCOUNT_NUMBER.~BAndika OAFID: $OAFID muri trial tracker yawe. ~B1)Gukomeza~B99)Kuvamo',
    },
    'dpm_reg_complete_sms': {
        'en': 'account number is $ACCOUNT_NUMBER oafid is $OAFID',
        'ki': 'Murakoze cyane kwiyandikisha muri TUBURA! Konte yawe ni $ACCOUNT_NUMBER. Yandike uyizirikane! Andika OAFID: $OAFID muri trial tracker yawe. Saba umuhinzi kubitsa',
    },
    'dpm_glus': {
        'en': 'enter GAB or KAM',
        'ki': 'Andika GAB cyangwa KAM',
    },
    'enr_order_already_finalized': {
        'en': 'order already finalized',
        'ki': 'Ibyo watumije byamaze kwemezwa',
    },
    'enr_invalid_nid_lookup': {
        'en': 'NID not found',
        'ki': 'Nomero ndangamuntu ntabwo ibonetsemo',
    },
    'enr_glus_sms': {
        'en': 'Your GL enroll code is $GLUS. Use this when registering clients into your group.',
        'ki': 'Nomero y\'umukuru w\'itsinda wifashisha utumiza ni:$GLUS',
    },
    'enr_prepayment': {
        'en': 'Prepayment: $PREP',
        'ki': 'Ubwishyu bwibanze: $PREP',
    },
    'rgo_prep_insufficient': {
        'en': 'prepayment insufficient',
        'ki': 'Niwatenze ubwishyu busabwa kugirango utumize. Urasaba kubutanga kugirango kugirango ubashe gutumiza',
    },
    'enr_order_period_finished': {
        'en': 'order period finished',
        'ki': 'enr_order_period_finished',
    },
    'chx_confirm': {
        'en': 'enter number of chickens to confirm',
        'ki': 'Muraho $NAME wemerewe inkoko $CHX_NUM. Ni zingahe ushaka kwemeza muri zo?',
    },
    'chx_confirmed': {
        'en': 'you have confirmed $CHX_NUM chickens. your code is $CONFIRMATION_CODE',
        'ki': 'Byagenze neza, umaze gutumiza inkoko  $CHX_NUM . Uyu niwo mubare $CONFIRMATION_CODE wemeza ko wazitumije',
    },
    'chx_not_confirmed': {
        'en': 'invalid input try again',
        'ki': 'Ibyo wahisemo ntibibashije kuboneka~B1) Gukomeza~B99) Kuvamo',
    },
    'chx_already_finalized': {
        'en': 'order already finalized',
        'ki': 'Ibyo watumije byamaze kwemezwa',
    },
    'chx_too_many': {
        'en': 'too many chickens selected',
        'ki': 'Inkoko wanditse zirenze izo wemerewe, shyiramo umubare',
    },
    'chx_final_confirm': {
        'en': 'you want to confirm $CHX_NUM chix. 1 to confirm',
        'ki': 'Muraho, wemerewe inkoko $CHX_NUM. Ni zingahe ushaka kwemeza muri zo?~B1) Gukomeza~B99) Kuvamo',
    },
    'chx_none_confirmable': {
        'en': 'you are not currently eligible to confirm any chickens',
        'ki': 'Ubu ntiwemerewe kwemeza inkoko n\'imwe',
    },
    'cor_payg_true': {
        'en': 'Your new PAYG code is $PAYG. Press any key to return to the main menu.',
        'ki': 'Umubare wo gucana itara ni $PAYG. Kanda # gusubira inyuma',
    },
    'cor_payg_false': {
        'en': 'You have not registered your SHS product yet. Please enter your product\'s serial number.',
        'ki': 'Ntabwo wandikishije itara ryawe. Injizamo umubare uranga itara kugirango uryandikishe',
    },
    'cor_payg_error': {
        'en': 'An error has occurred and has been reported to our technical team.',
        'ki': 'Habayemo ikibazo kandi cyoherejwe ku ikipe ishinzwe kugikemura.',
    },
    'cor_payg_invalid_serial': {
        'en': 'You have entered an invalid serial number. Please try again.',
        'ki': 'Nomero iranga itara wanditse si yo. Ongera ugerageze',
    },
    'cor_payg_insufficient': {
        'en': 'Your balance is not enough to get a new PAYG code.',
        'ki': 'Nturarangiza kwishyura ngo uhabwe code nshya.',
    },
    'cor_payg_duplicate': {
        'en': 'You have registered multiple SHS products. Please enter the serial number of the product you\'d like to use.',
        'ki': 'Wanditseho amatara menshi. Andikamo nomero iranga itara uri gukoresha.',
    },
    'enr_autofinalize': {
        'en': 'The Tubura order for account $ACCOUNT has been finalized. Please call 2580 if you need any assistance.',
        'ki': 'Ibyo watumije muri Tubura kuri Konti $ACCOUNT byararakozwe. Ukeneye ubufasha hamagara 2580',
    },
    'enr_missing_glvv': {
        'en': 'You have not registered with a group. Please enter your group ID.',
        'ki': 'Ntago wanditse muri iri tsinda. Shyiramo numero y\'itsinda ryawe.',
    },
    'enr_incorrect_glvv': {
        'en': 'You have entered an incorrect group ID. Please try again.',
        'ki': 'Wanditse numero y\'itsinda itari yo. Ongera ugerageze',
    },
    'enr_continue': {
        'en': 'Your account will be assigned to group ID $GROUP. Please enter any key to continue.',
        'ki': 'Konti yawe iri mu itsinda numero y\'itsinda $GROUP. Kanda # Gukomeza',
    },
    'enr_add_groupname': {
        'en': 'Please enter your group name.',
        'ki': 'Andika izina ry\'itsinda ryawe',
    },
    'enr_repayment_alert': {
        'en': 'Pay $REMAINDER to reach 60% of your 20A credit for you to pick up your 20B order',
        'ki': 'Ishyura $REMAINDER abura kugirango ugere kuri 60% y\'ideni ryawe rya 20A kugirango wemererwe gufata ibyo utumije',
    },
    'enr_prepayment_alert': {
        'en': 'Your prepayment: $PREPAYMENT',
        'ki': 'Ubwishyu bwibanze: $PREPAYMENT',
    },
    'chx_order_message': {
        'en': 'Hello $NAME you are eligible for $CHX_NUM number of chickens. How many would you like to confirm?',
        'ki': 'Muraho $NAME wemerewe inkoko $CHX_NUM. Ni zingahe ushaka kwemeza muri zo?',
    },
    'chx_invalid_order': {
        'en': 'Sorry, you put in an invalid number. Your order must be between 2 and $CHX_NUM chickens. Please try again',
        'ki': 'Ushyizemo umubare utari wo, wemerewe kwemeza inkoko  ziri hagati ya 2 na $CHX_NUM. Ongera ugerageze',
    },
    'chx_confirm_order': {
        'en': 'Please confirm, you would like $ORDER chicken.  If you press yes you will be increasing your total credit by $CREDIT and you will get your chicken in $MONTH. To confirm press 1',
        'ki': 'Emeza niba ukeneye inkoko $ORDER. Niba ukanze kuri Yego, amafaranga $CREDIT yiyongereye ku ideni. ~B1) Yego~B2) Oya',
    },
    'chx_order_finalized': {
        'en': 'Congratulations, you have ordered $ORDER chicks.  This is your voucher code: $VOUCHER. You will receive a follow up SMS with this number, but just in case, write it down now!  If you lose this number you will be able to access it later using your account number.',
        'ki': 'Byagenze neza, umaze gutumiza inkoko  $ORDER. Uyu niwo mubare $VOUCHER wemeza ko wazitumije: Wandike ahantu ku ruhande wibuke ko ariwo uzerekana ugiye gufata inkoko kwa Agenti wanyu.',
    },
    'chx_confirmation_sms': {
        'en': 'You have confirmed $ORDER chickens. Your code is $VOUCHER.  You will receive your chickens in 2 months. See your FO for details',
        'ki': 'Byagenze neza, umaze kwemeza inkoko $ORDER. Uyu niwo mubare $VOUCHER wemeza ko wazemeje. Uzafata inkoko nyuma y\'amezi 2. Ibindi bisobanuro wabaza Umukangurambaga wawe.',
    },
    'chx_not_eligible': {
        'en': 'You are eligible for 0 chickens. Please increase your repayment and confirm.',
        'ki': 'Ubu ntiwemerewe kwemeza inkoko nimwe. Ishyura ugere ku ntego usabwa wemeza.',
    },
    'chx_change_order': {
        'en': 'Hello $NAME. You have already confirmed $ORDER number of chicken, would you like to change your number of confirmed chicken?~B1) Yes~B2) No',
        'ki': 'Muraho $NAME. Wamaze kwemeza inkoko $ORDER, urifuza guhindura umubare w\'inkoko wemeje?~B1) Yego~B2) Oya',
    },
    'chx_invalid_order_opt2': {
        'en': 'Sorry, you put in an invalid number. You are eligible for 2 chickens only. Please try again.',
        'ki': 'Ushyizemo umubare utariwo. Wemerewe inkoko 2 gusa. Ongera ugerageze.',
    },
    'imp_closing_message': {
        'en': 'Thank you for answering the questions. You answered $NUM_CORRECT out of 8 correctly.',
        'ki': 'Murakoze gusubiza ibibazo. Mubibazo mwakoze mubashyije gusubiza $NUM_CORRECT kuri 8 neza.',
    },
    'imp_main_splash': {
        'en': 'Welcome to the impact quiz. Please select your location to continue. ~B$GEO_MENU',
        'ki': 'Murakaza neza. ~B$GEO_MENU',
    },
    'quiz_response': {
        'en': '$FEEDBACK~B$TEXT~B$OPT1~B$OPT2~B$OPT3~B$OPT4~B5)$OPT5',
        'ki': '$FEEDBACK~B$TEXT~B$OPT1~B$OPT2~B$OPT3~B$OPT4~B$OPT5',
    },
    'imp_invalid_geo': {
        'en': 'Invalid input; please try again ~B',
        'ki': 'Ibyo wahisemo ntibibashije kuboneka- ongera ugererageze ~B',
    },
    'pin_verification': {
        'en': 'Please enter your PIN, or reply with "99" to set a new PIN.',
        'ki': 'Shyiramo umubare w\'ibanga, cyangwa ukande 99 uhange umubare w\'ibanga mushya.',
    },
    'pin_attempts_exceeded': {
        'en': 'You have exceeded the number of attempts to enter a PIN. Please call the hotline at 2580 for assistance.',
        'ki': 'Warengeje incuro zo kwibeshya umubare w\'ibanga. Hamagara ku murongo mugari 2580 uhabwe ubufasha',
    },
    'pin_incorrect_pin': {
        'en': 'The entered PIN is not correct. Please try again, or enter "99" to set a new PIN.',
        'ki': 'Umubare w\'ibanga ushyizemo si wo. Kanda 99 ushyiremo umubare w\'ibanga mushya.',
    },
    'pin_reset': {
        'en': 'Enter your new PIN (must be 4 numbers long).',
        'ki': 'Shyiramo umubare w\'ibanga(ni imibare ine gusa)',
    },
    'pin_confirmed': {
        'en': 'Your new PIN is $PIN.',
        'ki': 'Umubare w\'ibanga mushya ni $PIN',
    },
    'pin_confirm_menu': {
        'en': 'You have entered $PIN. 1) Set this as your PIN 2) Enter a different PIN',
        'ki': 'Umubare w\'ibanga winjije ni $PIN. ~B1) Wemeze uwuzirikane ~B2) Hitamo undi nimba udashaka uwo',
    },
    'pin_security_attempts_exceeded': {
        'en': 'You have exceeded the number of attempts allowed. Please call 2580 for assistance.',
        'ki': 'Warengeje incuro zo kwibeshya. Hamagara ku murongo mugari 2580 uhabwe ubufasha',
    },
    'pin_incorrect_response': {
        'en': 'You have entered an incorrect response. Please try again.',
        'ki': 'Washyizemo imibare itari yo. Ongera ugerageze',
    },
    'pin_unset': {
        'en': 'You have not set a PIN. Please do the following steps to set your PIN.',
        'ki': 'Nta mubare w\'ibanga ufite. Kurikiza amabwiriza wishyirireho umubare w\'ibanga',
    },
    'pin_security_question3': {
        'en': 'Please write the initials of your oldest child. Reply with "99" if you don\'t have any children.',
        'ki': 'Andika inyuguti ibanza kuri buri zina ry\'umwana wawe mukuru. Kanda 99 nimba ntamwana ufite.',
    },
    'pin_invalid_sq1': {
        'en': 'You have entered an invalid year. Your format should be "20XX". Please try again.',
        'ki': 'Wanditse nabi umwaka ubajijwe andika "20XX". Ongera birakunda',
    },
    'pin_security_message': {
        'en': 'We will now ask you three questions to verify your identity so that we can protect your account. Reply with # to continue.',
        'ki': 'Tugiye kukubaza ibibazo 3 by\'imyirondoro yawe bityo byongerere konti yawe umumutekano. Kanda # ukomeze',
    },
    'pin_security_question1': {
        'en': 'Please enter the first year when you started with TUBURA. The format should be "20XX".',
        'ki': 'Ni muwuhe mwaka wafashe bwambere ibyo TUBURA itanga? Ni muwuhe mwaka wafashe bwambere ibyo TUBURA itanga? Umwaka ubajijwe andika "20XX".',
    },
    'pin_security_question2': {
        'en': 'Please type the first three letters of your group name.',
        'ki': 'Andika inyuguti 3 zibanza z\'izina ry\'Itsinda rya TUBURA wanditsemo',
    },
    'train_main_splash': {
        'en': 'Welcome to TUBURA training. Pease select one in the following division ~B$Division_MENU',
        'ki': 'Murakaza neza muri TUBURA training. Hitamo division imwe muzikurikira ~B$Division_MENU',
    },
    'mml_display_agents': {
        'en': 'Agents in your site: ~B$GEO_MENU',
        'ki': 'Aba ajenti ba MM bari mu kagari kawe: ~B$GEO_MENU',
    },
    'mml_agent_display': {
        'en': 'Name:  $NAME ~BPhone number 1: $PN1 ~BNetwork: $NETWORK',
        'ki': 'Amazina: $NAME ~BInimero ya telefone 1: $PN1 ~BUmuyoboro w\' itumanaho: $NETWORK',
    },
    'mml_invalid_geo_input': {
        'en': 'Invalid input - please try again',
        'ki': 'Ibyo wahisemo ntibibashije kuboneka - ongera ugererageze',
    },
    'train_type_splash': {
        'en': 'Please choose one in the following trainings ~B$Type_MENU',
        'ki': 'Hitamo imwe murizi zikurikira ~B$Type_MENU',
    },
    'training_province_splash': {
        'en': 'Please select your province ~B$GEO_MENU',
        'ki': 'Hitamo intara ~B$GEO_MENU',
    },
    'training_district_splash': {
        'en': 'Please select your district ~B$GEO_MENU',
        'ki': 'Hitamo Akarere ~B$GEO_MENU',
    },
    'training_closing_message': {
        'en': '$FEEDBACK. Thank you for answering the questions. You have answered $NUM_CORRECT correctly',
        'ki': '$FEEDBACK. Murakoze gusubiza ibibazo. Mu bibazo mwasubije,  $NUM_CORRECT nibyo mwasubije neza neza',
    },
    'training_invalid_input': {
        'en': 'You have entered an invalid input. Please try again',
        'ki': 'Winjije umubare utariwo ongera ugerageze',
    },
    'training_exit_message': {
        'en': 'You choosed to end the survey. You will continue from where you reached if you come back in 15 minutes.',
        'ki': 'Muhisemo kurekera. Murakomereza aho mwari mugeze nimugaruka muminota 15.',
    },
    'training_sector_splash': {
        'en': 'Please select your sector~B$GEO_MENU',
        'ki': 'Hitamo umurenge ~B$GEO_MENU',
    },
    'training_site_splash': {
        'en': 'Please select your site ~B$GEO_MENU',
        'ki': 'Hitamo site yawe ~B$GEO_MENU',
    },
    'wrong_answer_feedback': {
        'en': 'Wrong: $ANS is the correct answer',
        'ki': 'Si byo: $ANS nicyo gisubizo',
    },
    'answer_feedback': {
        'en': '$ANS is the correct answer',
        'ki': '$ANS nicyo gisubizo',
    },
    'enr_nid_client_confirmation': {
        'en': 'You have entered $ENR_NID_CONFIRM , is this your national id number ? ~B $ENR_CONFIRMATION_MENU',
        'ki': 'Mwinjije $ENR_NID_CONFIRM . Ese iyi niyo numero y\' irangamuntu yanyu? ~B $ENR_CONFIRMATION_MENU',
    },
    'mml_invalid_geo': {
        'en': 'We do not have information for agents in your area yet. Our team has been informed of your request.',
        'ki': 'Ntamakuru turamenya yaba ajenti mu gace uherereyemo. Ikipe yacu yamenyeshejwe icyifuzo cyawe.',
    },
    'enr_group_id_confirmation': {
        'en': 'You have entered $ENR_GROUP_ID, named $NAME. Is this your group ? ~B $ENR_CONFIRMATION_MENU',
        'ki': 'Mwinjije $ENR_GROUP_ID yitwa: $NAME. Ese niryo tsinda ryanyu ? ~B $ENR_CONFIRMATION_MENU',
    },
    'enr_group_id_info': {
        'en': 'district : $DISTRICT at site : $SITE and group: $GROUP',
        'ki': 'akarere ka $DISTRICT kuri site: $SITE, itsinda ryitwa: $GROUP',
    },
    'enr_confirmation_menu': {
        'en': '1) Yes, it is right ~B 2) No, it\'s not right',
        'ki': '1) Yego, nibyo ~B 2) Oya, sibyo',
    },
    'invalid_group_id': {
        'en': 'The group id entered does not exist',
        'ki': 'Numero y\'itsinda mwinjije ntabwo ibaho',
    },
    'cor_enr_main_splash': {
        'en': 'Welcome to TUBURA. Please enter your account number or \n1) To regiter\n2) Non Client Menu',
        'ki': 'Murakaza neza muri TUBURA. Andika nimero ya konti y\'umuhinzi cyangwa \n1) Kwiyandikisha\n2) Niba utari umukiriya',
    },
    'main_menu': {
        'en': 'Welcome to One Acre Fund Tubura.\n1) Not currently a client\n2) Already a client',
        'ki': 'Urakaza neza muri Tubura. Andika\n1) Niba udasanzwe uri umukiriya wa Tubura\n2) Niba usanzwe uri umukiriya wa Tubura'
    },
    'ENR_FINALIZE_TERMS_AND_CONDITION': {
        'en': 'I enroll in TUBURA by agreeing on TUBURA loan Terms and Conditions and I will sign it on the day of input pickup.1) Continue ~B 2) Quit',
        'ki': 'Niyandikishije muri TUBURA nemeye amabwiriza n\'amategeko agenga ideni rya TUBURA nkazayasinya kumunsi w\'ifata ry\'inyongeramusaruro.1) Gukomeza ~B 2)Gusohoka',
    },
    'FAILURE_REGISTERING': {
        'en': 'Registration failed. Please try again later',
        'en-ke': 'Registration failed, Please try again later. At this time many people are using the system!',
        'ki': 'Kwiyandikisha biranze. Ongera mukanya',
        'sw': 'Tafadhali jaribu tena baada ya mda kidogo. Watu wengi wanatuma maombi kwa sasa!'
    },
    'ENR-SMS-ORDER-FINALIZE': {
        'en': 'The total price is $TotalPrice for your $OrderDetails orders, don’t forget to make a prepayment by the 1st of July. Tubura is your trusted partner for quality products and quality harvests.',
        'ki': 'Watumije $OrderDetails, k\'ideni ringana na $TotalPrice, kandi ntuzibagirwe gutanga ubwishyu bw\'ibanze mbere ya 1 Nyakanga. TUBURA umufatanyabikorwa wawe wizewe, dutanga inyongeramusaruro zitanga UMUSARURO MWIZA.',
    },
    'enr-group-constitution-approvement': {
        'en': 'I understood Group Constitution rules and I agree to follow them. I will sign these rules on input pickup day. Reply 1 to continue',
        'ki': 'Nemeye gukurikiza amategeko y\'itsinda. Nzayashyiraho umukono ku munsi wo gufata inyongeramusaruro. Subiza 1 gukomeza',
    },
    'FAILURE_REGISTERING_NATIONAL_ID_CONFLICT': {
        'en': 'Your national id is already registered',
        'ki': 'Kwiyandikisha biranze kuko numero yirangamuntu iranditse',
    },
    'no_quiz_menu': {
        'en': 'Sorry, no active quiz currentlly',
        'ki': 'Nta suzumabumenyi ihari ubu',
    },
    'enr_not_finalized': {
        'en': 'Ordering not finalized',
        'ki': 'Itumiza ntago ryemejwe',
    },
    'ENR_NATIONAL_ID_DUPLICATE': {
        'en': 'The entered national ID is already in our system. Please register with a different national ID or place an order using your account number',
        'ki': 'Inomero y Irangamuntu mwinjije iranditse muri sisiteme yacu. Koresha iyindi numero y Irangamuntu cyangwa utumize ukoresheje nimero ya konti yumuhinzi',
    },
    'market_persons_in_group': {
        'en': 'Number of farmers' ,
        'ki': 'Umubare w\'abahinzi'
        
    },
    'maize_number': {
        'en': 'Quantity of this unshelled maize to sell in bags (use big bags of 100kg)',
        'ki': 'Imifuka y\'ibigori bidahunguye ugurisha(hakoreshwa umufuka munini w\'ibiro 100)'
    },
    'pshops_main_splash': {
        'en': 'Welcome to the PShop Client portal. Please enter your Account Number\n0: to locate your Agrodealer partner',
        'ki': 'Murakaza neza ku iduka. Injizamo konti yawe y\'umuhinzi\n0: Kubona umufatanyabikorwa ukwegereye',
    },
    'incorrect_account_number': {
        'en': 'Incorrect input. Please enter your Account Number',
        'ki': 'Winjije umubare utariwo. Ongera winjze umubare wa konti yawe ukoresha wishyura iduka',
    },
    'pshop_main_menu': {
        'en': 'Welcome $NAME ~B1 - To check balance ~B2 - Solar Codes',
        'ki': 'Murakaza neza $NAME ~B1. Kureba uko ubwishyu buhagaze ~B2. Kodi y\'itara',
    },
    'main_message': {
        'en': 'Paid: $REPAY ~BTotal credit: $CREDIT ~BRemaining: $BALANCE ~B1 - Back to main menu',
        'ki': 'Ayishyuwe: $REPAY ~BIdeni ryose: $CREDIT ~BIdeni risigaye: $BALANCE ~B1 - Subira inyuma',
    },
    'solar_unlocked': {
        'en': 'You registered Serial $SERIAL ~BUnlock code: $ACTCODE ~B99 - Back to menu',
        'ki': 'Umubare uranga itara ryawe ni: $SERIAL ~BKodi yawe ya burundu ni: $ACTCODE ~B99. Subira inyuma',
    },
    'solar_locked': {
        'en': 'You registered Serial $SERIAL ~BLast code: $ACTCODE ~B1- New code ~B99 - Back to menu',
        'ki': 'Umubare uranga itara ryawe ni: $SERIAL ~BKodi uheruka kubona ni: $ACTCODE ~B1. Kodi nshya ~B99. Subira inyuma',
    },
    'solar_nonreg': {
        'en': 'You have not registered an SHS. Enter your Serial Number to register ~B99 - Back to menu',
        'ki': 'Ntabwo wandikishije itara ryawe. Injizamo umubare uranga itara kugirango uryandikishe ~B99. Subira inyuma',
    },
    'insufficient_funds': {
        'en': 'You have not paid enough to request your new code ~BTo the next code: $REMAIN_BAL ~BTo unlock: $BALANCE ~B1 - Back to main',
        'ki': 'Ubwishyu bwawe ntibuhagije. Ubusigaye ngo ubone kodi ~BNshya: $REMAIN_BAL ~BYa burundu: $BALANCE ~B1. Subira ahabanza',
    },
    'unlock_success': {
        'en': 'You have successfully unlocked your SHS! Your unlock code is: $ACTCODE ~B1 - Back to main',
        'ki': 'Warangije kwishyura neza itara ryawe! Kodi ya burundu ni: $ACTCODE ~B1. Subira ahabanza',
    },
    'activation_code': {
        'en': 'Your new activation code is: $ACTCODE ~B1 - Back to main',
        'ki': 'Kodi yawe nshya ni: $ACTCODE ~B1. Subira ahabanza',
    },
    'reg_success': {
        'en': 'You have registered. Your activation code is $ACTCODE ~B99 - Back to menu',
        'ki': 'Kwandikisha itara ryawe byagenze neza. Kodi yo gucana itara ryawe ni: $ACTCODE ~B99. Subira inyuma',
    },
    'already_reg': {
        'en': 'Serial number already registered please try again ~B99 - Back to menu',
        'ki': 'Umubare uranga itara winjije warafashwe. Ongera ugerageze ~B99. Subira inyuma',
    },
    'serial_not_found': {
        'en': 'This serial number is not found please try again ~B99 - Back to menu',
        'ki': 'Umubare uranga itara ushyizemo ntubaho. Ongera ugerageze ~B99. Subira inyuma',
    },
    'client_alert': {
        'en': 'An error has occurred and has been reported to our technical team.',
        'ki': 'Habayemo ikibazo kandi cyoherejwe ku ikipe ishinzwe kugikemura.',
    },
    'invalid_input': {
        'en': 'Incorrect input. ~B1 - To check balance ~B2 - Solar Codes',
        'ki': 'Winjije umubare utariwo. ~B1. Kureba uko ubwishyu buhagaze ~B2. Kodi y\'itara'
    },
    'solar_duplicate': {
        'en': 'You have registered multiple products. Please enter the serial number of the product you\'d like to use.',
        'ki': 'Wanditseho amatara menshi. Andikamo nomero iranga itara uri gukoresha.',
    },
    'view_group_repayment': {
        'en': '$label) View group repayment\n',
        'ki': '$label) Reba amakuru y\'ubwishyu bwishyuwe mu itsinda\n'
    },
    'enrolled_national_id': {
        'en': 'Thank you for expressing your interest to enroll with OAF. Your Account Number is: $AccountNumber .Your FO will reach out to you to add inputs to your order.',
        'en-ke': 'Thank you for expressing your interest to enroll with OAF. Your Account Number is: $AccountNumber .Your FO will reach out to you to add inputs to your order.',
        'sw': 'Asante kwa nia ya kujiandikisha na OAF. Nambari yako ya akaunti ni: $AccountNumber. Mwalimu wa nyanjani atawasiliana na wewe ili uchagua bidhaa.',
        'ki': 'numero y\'irangamuntu yanyu yanditse kuri konte ya TUBURA: $AccountNumber . Mukoreshe iyo mutumiza' 
    },
    'end_review_msg': {
        'en': '1)Choose another bundle/order\n2)Confirm order\n3) go back',
        'ki': '1)Hitamo ikindi utumiza\n2)Emeza gutumiza\n3)Subira ahabanza'
    },
    'no_produts_msg': {
        'en': 'No order placed so far\n1)Choose a bundle/order\n3) go back',
        'ki': 'Ntacyo uratumiza\n1) Tumiza\n3)Subira ahabanza '
    },
    'nonClientMenu': {
        'en': 'Welcome\n1) Sell harvest',
        'ki': 'Murakaza neza\n1) Kugurisha Umusaruro'
    },
    'are_you_a_gl': {
        'en': 'Are you the Group leader?\nPress 1 if you are\nPress 2 if you are not',
        'ki': 'Uri umukuru w’itsinda?\nKanda 1 niba uri we\nKanda 2 niba utariwe'
    },
    'enrollment_ended': {
        'en': 'Enrollment period endend for your district',
        'ki': 'Igihe cyo gutumiza cyararangiye mu karere kanyu'
    },
    'enr_reg_complete_msg': {
        'en': 'Thank you for enrolling with TUBURA! Your account number is $ACCOUNT_NUMBER. Please save this!',
        'ki': 'Murakoze kwiyandikisha muri TUBURA, $NAME! Konte yawe ni $ACCOUNT_NUMBER. Yandike uyizirikane!',
    }
};