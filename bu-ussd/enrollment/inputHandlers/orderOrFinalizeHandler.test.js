...
describe('order or finalize input handler', () => {
    it('should reprompt on empty input', () => {
        ...
        const handler = orderOrFinalizeHandler.getHandler('en_bu', onOrderOrFinaliseSelected);
        handler();
    ...
    it('should call onOrderOrFinaliseSelected if input is 1', () => {
        ...
        const handler = orderOrFinalizeHandler.getHandler('en_bu', onOrderOrFinaliseSelected);
        handler('1');
        expect(onOrderOrFinaliseSelected).toHaveBeenCalledWith('en_bu', '1');
    });
    it('should call onOrderOrFinaliseSelected if input is 2', () => {
        const onOrderOrFinaliseSelected = jest.fn();
        const handler = orderOrFinalizeHandler.getHandler('en_bu', onOrderOrFinaliseSelected);
        handler('2');
        expect(onOrderOrFinaliseSelected).toHaveBeenCalledWith('en_bu', '2');
    });
