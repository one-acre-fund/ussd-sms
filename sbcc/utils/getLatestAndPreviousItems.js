...
module.exports = function (items, currentDate) {
    ...
    if (!(currentDate instanceof Date) || isNaN(currentDate.getTime())) throw new TypeError('currentDate is not a valid date');
    ...
    while(start <= end) {
        ...
        if (mid === items.length - 1 && currentDateTime >= currentItemDateTime) {
            result.latest = items[mid].name;
            index = mid;
            break;
        }
